...
import { formatValue, next, prev, VALUE_FORMAT } from "./datepicker.utils";
...
export function Calendar({
  ...
  let { displayActive } = { ...defaultOptions, ...optionsProp };
  // let { activeDate, actions } = useCalendar(props);

  let days = React.useMemo(() => getDaysInMonth(activeDate), [activeDate]);
  return (
    <div
    ...
    >
      ...
      <div className="calendar__date-grid">
        {days.map((d, index) => {
          let current = d.format(VALUE_FORMAT);
          let isActive = current === activeDate;
